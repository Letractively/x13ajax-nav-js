...
if(el.prop("tagName")==="IMG"&&el.parent().prop("tagName")==="A")el=_this.jQuery(el.parent());if(el.prop("tagName")!=="A")return true;var href=el.prop("href");if(!_this.isBaseURL(href))return true;if(!el.hasClass("x13AjaxNav")&&!el.parent().hasClass("x13AjaxNav"))return true;if(_this.processInternalLink(href))history.pushState(null,null,href);e.stopPropagation();return false};this.reloaded=function(){var k;for(var k in _this.rebinds["*"])_this.rebinds["*"][k]();indBind=_this.params.controller.toLowerCase();
